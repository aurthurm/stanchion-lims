<template>
  <div>
    <h2 id="page-heading" data-cy="CountryHeading">
      <span v-text="$t('stanchionApp.country.home.title')" id="country-heading">Countries</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('stanchionApp.country.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CountryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('stanchionApp.country.home.createLabel')"> Create a new Country </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && countries && countries.length === 0">
      <span v-text="$t('stanchionApp.country.home.notFound')">No countries found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="countries">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('stanchionApp.country.name')">Name</span></th>
            <th scope="row"><span v-text="$t('stanchionApp.country.province')">Province</span></th>
            <th scope="row"><span v-text="$t('stanchionApp.country.province')">Province</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="country in countries" :key="country.id" data-cy="entityTable">
            ...
            <td>{{ country.name }}</td>
            <td>
              <div v-if="country.province">
                <router-link :to="{ name: 'ProvinceView', params: { provinceId: country.province.id } }">{{
                  country.province.id
                }}</router-link>
              </div>
            </td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="stanchionApp.country.delete.question" data-cy="countryDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-country-heading" v-text="$t('stanchionApp.country.delete.question', { id: removeId })">
