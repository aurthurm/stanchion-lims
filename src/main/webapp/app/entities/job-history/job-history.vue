<template>
  <div>
    <h2 id="page-heading" data-cy="JobHistoryHeading">
      <span v-text="$t('stanchionApp.jobHistory.home.title')" id="job-history-heading">Job Histories</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('stanchionApp.jobHistory.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'JobHistoryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('stanchionApp.jobHistory.home.createLabel')"> Create a new Job History </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && jobHistories && jobHistories.length === 0">
      <span v-text="$t('stanchionApp.jobHistory.home.notFound')">No jobHistories found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="jobHistories">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('stanchionApp.jobHistory.startDate')">Start Date</span></th>
            <th scope="row"><span v-text="$t('stanchionApp.jobHistory.endDate')">End Date</span></th>
            <th scope="row"><span v-text="$t('stanchionApp.jobHistory.language')">Language</span></th>
            <th scope="row"><span v-text="$t('stanchionApp.jobHistory.job')">Job</span></th>
            <th scope="row"><span v-text="$t('stanchionApp.jobHistory.department')">Department</span></th>
            <th scope="row"><span v-text="$t('stanchionApp.jobHistory.employee')">Employee</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="jobHistory in jobHistories" :key="jobHistory.id" data-cy="entityTable">
            ...
            <td v-text="$t('stanchionApp.Language.' + jobHistory.language)">{{ jobHistory.language }}</td>
            <td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="stanchionApp.jobHistory.delete.question" data-cy="jobHistoryDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-jobHistory-heading" v-text="$t('stanchionApp.jobHistory.delete.question', { id: removeId })">
