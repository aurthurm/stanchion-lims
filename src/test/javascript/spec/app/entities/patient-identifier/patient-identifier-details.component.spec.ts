/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import VueRouter from 'vue-router';

import * as config from '@/shared/config/config';
import PatientIdentifierDetailComponent from '@/entities/patient-identifier/patient-identifier-details.vue';
import PatientIdentifierClass from '@/entities/patient-identifier/patient-identifier-details.component';
import PatientIdentifierService from '@/entities/patient-identifier/patient-identifier.service';
import router from '@/router';
import AlertService from '@/shared/alert/alert.service';

const localVue = createLocalVue();
localVue.use(VueRouter);

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
localVue.component('font-awesome-icon', {});
localVue.component('router-link', {});

describe('Component Tests', () => {
  describe('PatientIdentifier Management Detail Component', () => {
    let wrapper: Wrapper<PatientIdentifierClass>;
    let comp: PatientIdentifierClass;
    let patientIdentifierServiceStub: SinonStubbedInstance<PatientIdentifierService>;

    beforeEach(() => {
      patientIdentifierServiceStub = sinon.createStubInstance<PatientIdentifierService>(PatientIdentifierService);

      wrapper = shallowMount<PatientIdentifierClass>(PatientIdentifierDetailComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: { patientIdentifierService: () => patientIdentifierServiceStub, alertService: () => new AlertService() },
      });
      comp = wrapper.vm;
    });

    describe('OnInit', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        const foundPatientIdentifier = { id: 123 };
        patientIdentifierServiceStub.find.resolves(foundPatientIdentifier);

        // WHEN
        comp.retrievePatientIdentifier(123);
        await comp.$nextTick();

        // THEN
        expect(comp.patientIdentifier).toBe(foundPatientIdentifier);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundPatientIdentifier = { id: 123 };
        patientIdentifierServiceStub.find.resolves(foundPatientIdentifier);

        // WHEN
        comp.beforeRouteEnter({ params: { patientIdentifierId: 123 } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.patientIdentifier).toBe(foundPatientIdentifier);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
