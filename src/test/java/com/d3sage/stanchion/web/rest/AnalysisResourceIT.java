...
class AnalysisResourceIT {
    ...
    void createAnalysis() throws Exception {
        ...
        restAnalysisMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(analysisDTO)))
    ...
    @Test
    @Transactional
    void putNewAnalysis() throws Exception {
        // Initialize the database
        analysisRepository.saveAndFlush(analysis);
        ...
        restAnalysisMockMvc
            .perform(
                put(ENTITY_API_URL_ID, analysisDTO.getId())
    ...
    void putWithMissingIdPathParamAnalysis() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAnalysisMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(analysisDTO)))
    ...
    void patchNonExistingAnalysis() throws Exception {
        ...
        restAnalysisMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, analysisDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(analysisDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamAnalysis() throws Exception {
        ...
        restAnalysisMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(analysisDTO))
            )
            .andExpect(status().isMethodNotAllowed());
